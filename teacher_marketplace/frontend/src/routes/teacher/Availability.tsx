  ...
  return (
    <div className="flex max-w-3xl flex-col gap-5">
      ...
      <section className="u-card u-card-pad">
        ...
        <p className="u-fine mt-0.5">Times are in {TZ}.</p>
        <div className="mt-4 flex flex-wrap items-end gap-3">
          <div className="u-field min-w-[10rem] flex-1">
            <label className="u-label" htmlFor="av-day">Day</label>
          ...
          <button type="button" className="u-btn-primary" onClick={addWindow} data-loading={adding || undefined} disabled={adding}>
            Add
          </button>
        </div>
      </section>
      ...
      <section className="u-card overflow-hidden">
        <div className="flex items-center justify-between gap-3 border-b border-ink-200 px-5 py-4">
          ...
          <span className="u-fine">{total} {total === 1 ? "window" : "windows"}</span>
        </div>
        ...
        {weekly.isLoading ? (
          ...
          <ul className="divide-y divide-ink-200">
            {byDay.map((d) => (
              <li key={d.n} className="flex items-start gap-4 px-5 py-3.5">
                ...
                {d.windows.length === 0 ? (
                  ...
                  <div className="flex flex-wrap gap-2">
                    {d.windows.map((w) => (
                      <span key={w.id}
                        className="inline-flex items-center gap-2 rounded-lg border border-pine-300 bg-pine-50 py-1 pl-3 pr-2 text-[0.8125rem] font-semibold tabular-nums text-pine-800">
    ...
    <section className="u-card u-card-pad">
      ...
      <div className="mt-4 flex flex-wrap items-end gap-3">
        ...
        <div className="u-field min-w-[12rem] flex-1">
          ...
          <input id="ex-why" className="u-input" placeholder="Travelling" value={reason} onChange={(e) => setReason(e.target.value)} />
        </div>
      ...
      {!loading && items.length > 0 && (
        <ul className="mt-4 flex flex-col gap-2 border-t border-ink-200 pt-4">
          {items.map((x) => (
            <li key={x.id} className="flex items-center justify-between gap-3 rounded-xl border border-ink-200 px-4 py-2.5">
              <span className="text-[0.9375rem] font-medium text-ink-900">
                {new Date(x.date).toLocaleDateString("en-IN", { day: "numeric", month: "long", year: "numeric" })}
                {x.reason && <span className="ml-2 font-normal text-ink-500">{x.reason}</span>}
              </span>
              <button type="button" className="u-btn-ghost u-btn-sm text-danger" onClick={() => remove(x)}>Remove</button>
            </li>
          ))}
        </ul>
      )}
