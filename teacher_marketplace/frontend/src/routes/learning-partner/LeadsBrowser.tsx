...
/**
 * Mirrors superadmin/LeadsBrowser.tsx exactly, scoped to this partner's own
 * referred students (GET /lp/students-lead-quality/, /lp/teacher-lead-reviews/)
 * - see apps.learning_partner.views.LPStudentLeadQualityView/
 * LPTeacherLeadReviewsView for the scoping rule (by student, not by teacher).
 */
...
function ByStudentTab() {
  ...
  const { data, isLoading, isError, error, refetch } = useQuery({
    queryKey: ["lp-students-lead-quality"],
    queryFn: () => api.get<{ results: StudentLeadQualitySummary[]; count: number }>("/lp/students-lead-quality/"),
  });
  const items = data?.results ?? [];

  const detail = useQuery({
    queryKey: ["lp-students-lead-quality", "detail", expanded],
    queryFn: () =>
      api.get<{ results: StudentLeadQualityRating[]; count: number }>("/lp/students-lead-quality/", {
  ...
  if (items.length === 0) {
    return <div className="u-card p-6 text-center text-ink-500">No lead-quality ratings recorded yet for your students.</div>;
  }
...
function ByTeacherTab() {
  ...
  const { data, isLoading, isError, error, refetch } = useQuery({
    queryKey: ["lp-teacher-lead-reviews", verdict],
    queryFn: () =>
      api.get<{ results: TeacherLeadReview[]; count: number }>("/lp/teacher-lead-reviews/", {
