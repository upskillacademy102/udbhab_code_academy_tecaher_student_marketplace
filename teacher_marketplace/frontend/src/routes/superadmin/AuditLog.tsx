...
const CATEGORIES = [
  ...
  { value: "admin_login", label: "Admin login approval" },
...
/**
 * Thin filtered/paginated wrapper over the existing GET /ops/events/ feed -
 * every privileged/security-relevant action on the platform, from staff
 * logins and failed attempts to bans, impersonation and admin approvals.
 * First frontend for this endpoint outside the old templates.
 */
export function AuditLog() {
  ...
  const { data, isLoading, isError, error, refetch } = useQuery({
    queryKey: ["ops-events", category, status, search, page],
    queryFn: () =>
      api.list<AuditLogEntry>("/ops/events/", {
        params: {
