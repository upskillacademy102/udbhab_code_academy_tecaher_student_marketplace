...
def make_learning_partner_admin(name: str = "TestPartner", *, mobile: str | None = None, **extra) -> User:
    """
    Create a Learning Partner admin: role=learning_partner (no department -
    see UserRole.LEARNING_PARTNER), with a generated-shape admin_account_name
    (the `n` suffix keeps it unique across tests that don't bother passing a
    distinct `name`). Mirrors what admin_account_naming.build_admin_account_name's
...
def login_lp(api_client, lp_admin, password: str = TEST_PASSWORD):
    """
    A Learning Partner signs in with its generated admin_account_name at
    its own dedicated endpoint, /staff/login-learning-partner/ - separate
    from both login()'s /auth/login/ path (rejects role=learning_partner)
    and the Admin account-name endpoint /staff/login-admin/ (which now
    rejects a Learning Partner account name outright - see
    StaffAdminLoginView / StaffLearningPartnerLoginView in
    apps.accounts.admin_api).
    """
    return api_client.post(
